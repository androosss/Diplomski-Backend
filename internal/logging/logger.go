...
func (l *Logger) WithRequestID(ctx context.Context) *zap.Logger {
	if ctx != nil {
		...
		return l.Z.With(zap.String("ReqID", ReqIDFromContext(ctx)))
	}
	return l.Z
}
...
func GetContextWithReqIDFromHeader(ctx context.Context, req *http.Request) context.Context {
	...
	// changed to req.Context() from context.Background() because with context.Background() we can't extract path parameters (eg. /{id})
